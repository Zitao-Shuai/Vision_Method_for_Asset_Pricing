...
if __name__ == "__main__":
    ...
    for i in range(k_fold):
        if args.dataset in vars(datasets):
            ...
            raise NotImplementedError
        train_dataset, test_dataset = dataset.tr_te_split(start = i * k_interval, end = (i + 1) * k_interval)
        dataset.reset_data(train_dataset)
        train_dataset = deepcopy(dataset)
        dataset.reset_data(test_dataset)
